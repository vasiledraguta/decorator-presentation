      ...
      <div className="flex h-full flex-col gap-6">
        ...
        <div className="flex flex-1 items-center gap-4">
          <div className="flex w-[45%] flex-col gap-6">
            <div
              className={`bg-bg-card border-border-card flex flex-col gap-4 rounded-xl border p-8 ${stagger(0, 1)}`}
            >
              ...
              <p className="text-text-muted text-xl leading-relaxed">
                Imagine building a UI toolkit (like the classic GoF example) where we want to modify
                a UI component to add traits like a{" "}
                <span className="text-text font-bold">Border</span> or{" "}
                <span className="text-text font-bold">Scrolling</span> to it.
              </p>
            </div>
            <div
              className={`border-red/20 bg-red/5 flex flex-col gap-4 rounded-xl border p-8 ${stagger(1, 1)}`}
            >
              <h3 className="text-red text-2xl font-bold">The Inheritance Trap</h3>
              <p className="text-text-muted text-lg leading-relaxed">
                We quickly encounter the problem of not being able to combine these traits without
                statically implementing a rigid subclass (e.g.,{" "}
                <code className="bg-red/10 text-red rounded px-2 py-1 font-mono text-base">
                  BorderedScrollableComponent
                </code>
                ).
              </p>
            </div>
          ...
          <div className="slide-enter-delay-2 flex w-[10%] justify-center">
            <ArrowRight className="text-accent h-16 w-16" />
          </div>
              ...
              return (
                <div
                  ...
                  className={`flex flex-col justify-center rounded-xl border px-8 py-5 transition-transform duration-500 ease-out ${stagger(idx, 3)} ${
                    isViolation
                      ? "border-red bg-red/10 z-10 scale-105 shadow-[0_0_40px_rgba(239,68,68,0.4)]"
                      : "border-red/20 bg-red/5 opacity-80"
                  }`}
                >
                  <h3
                    className={`mb-1 font-bold ${isViolation ? "text-red flex items-center gap-2 text-2xl" : "text-red text-xl"}`}
                  >
                    {isViolation && <AlertTriangle className="h-6 w-6" />}
                    {p.label}
                  </h3>
                  <p
                    className={`leading-relaxed ${isViolation ? "text-text text-xl font-medium" : "text-text-muted text-lg"}`}
                  >
                    {p.desc}
                  </p>
                </div>
...
export function SolutionSlide() {
  return (
    <SlideLayout slideNumber={4} sectionLabel="THE CORE PROBLEM">
      <div className="flex h-full flex-col items-center justify-center gap-8 pb-4 text-center">
        <div className="slide-enter border-border-card/50 bg-bg-card/20 flex h-72 w-full max-w-4xl shrink-0 items-center justify-center overflow-hidden rounded-xl border p-4 shadow-xl">
          <img
            src="/solution.png"
        ...
        <div className="slide-enter-delay-3 flex shrink-0 items-center gap-8">
          {[
